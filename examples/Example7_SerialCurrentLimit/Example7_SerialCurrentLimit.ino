/*
  ...
  This example turns the motor back and forth at different current limit settings.
  
  Things to note:
    - current limit can only be adjusted while in SERIAL MODE
    - stepSerial must be called for the setting change to take effect.
    - This effects current limit on both coils (A and B)
    - currentLimit is a 10-bit value, so must be within 0-1023
    - currentLimit is a percentage of VREF and also limited by torque setting
      (so you may want to set your "CUR ADJ" trimpot to max, and leave torque at 100% default)
...
void setup() {
  Serial.begin(115200);
  Serial.println("SparkFun ProDriver TC78H670FTG Example 7");
...
void loop() {
  myProDriver.setCurrentLimit(1023); // max current
  myProDriver.stepSerial(200, 0); // turn 200 steps, CW direction
  delay(1000);
  myProDriver.setCurrentLimit(512); // 50% current
  myProDriver.stepSerial(200, 0); // turn 200 steps, CW direction
  delay(1000);
}
